...
const callState = {
  ...
  roomId: null,
  localStream: null,
  pcs: {},       // uid → RTCPeerConnection
  listeners: []  // RTDB listener refs to detach on end
};

async function startCall() {
  if (!rtdb) { toast('Firebase not connected.', 'error'); return; }
  try {
    callState.localStream = await navigator.mediaDevices.getUserMedia({ video: true, audio: true });
  } catch (e) {
    try {
      callState.localStream = await navigator.mediaDevices.getUserMedia({ video: false, audio: true });
      toast('Camera unavailable — joining with audio only.', 'info');
    } catch (e2) {
      toast('Could not access microphone: ' + e2.message, 'error'); return;
    }
  }
  ...
  await rtdb.ref(`calls/${roomId}`).set({
    status: 'active', createdBy: auth.currentUser.uid,
    startedAt: firebase.database.ServerValue.TIMESTAMP
  });
  await rtdb.ref(`calls/${roomId}/participants/${auth.currentUser.uid}`).set({
    name: getCurrentUser()?.displayName || 'Unknown',
    joinedAt: firebase.database.ServerValue.TIMESTAMP
  });
  // Announce to group
  await db.collection('activeCall').doc('current').set({
    roomId, active: true, startedBy: auth.currentUser.uid,
    starterName: getCurrentUser()?.displayName || 'Unknown',
    startedAt: firebase.firestore.FieldValue.serverTimestamp()
  });

  if (window.onCallStarted) window.onCallStarted(roomId, callState.localStream, true);
  _listenForSignals(roomId);
  _listenForParticipants(roomId);
}
...
async function joinCall(roomId) {
  ...
  // Listen for signals BEFORE announcing presence
  _listenForSignals(roomId);
  ...
  snap.forEach(child => {
    if (child.key !== auth.currentUser.uid) _initPeer(roomId, child.key, true);
  });

  if (window.onCallStarted) window.onCallStarted(roomId, callState.localStream, false);
  _listenForParticipants(roomId);
}

function _listenForParticipants(roomId) {
  const ref = rtdb.ref(`calls/${roomId}/participants`);
  const handler = ref.on('child_added', snap => {
    const uid = snap.key;
    if (uid === auth.currentUser.uid || callState.pcs[uid]) return;
    // They joined after us — we wait for their offer (they initiate to us)
    _listenForSignals(roomId);
    if (window.onParticipantJoined) window.onParticipantJoined(uid, snap.val().name);
  });
  callState.listeners.push({ ref, handler, event: 'child_added' });

  const rmvRef = rtdb.ref(`calls/${roomId}/participants`);
  rmvRef.on('child_removed', snap => {
    const uid = snap.key;
...
async function _initPeer(roomId, theirUid, initiator) {
  ...
  pc.ontrack = e => {
    ...
    if (window.onRemoteStream) window.onRemoteStream(theirUid, remoteStream);
  };

  const myUid = auth.currentUser.uid;
  const pairKey = myUid < theirUid ? `${myUid}_${theirUid}` : `${theirUid}_${myUid}`;
  const sigRef = rtdb.ref(`calls/${roomId}/signals/${pairKey}`);

  if (initiator) {
    const offer = await pc.createOffer();
    await pc.setLocalDescription(offer);
    await sigRef.update({ offer: { sdp: offer.sdp, type: offer.type }, offerer: myUid });

    pc.onicecandidate = e => {
      if (e.candidate) sigRef.child('offerCandidates').push(e.candidate.toJSON());
    };

    sigRef.on('value', async snap => {
      const d = snap.val();
    ...
    sigRef.child('answerCandidates').on('child_added', async snap => {
      try { await pc.addIceCandidate(new RTCIceCandidate(snap.val())); } catch {}
    });
  }
}

function _listenForSignals(roomId) {
  const myUid = auth.currentUser.uid;
  rtdb.ref(`calls/${roomId}/signals`).on('child_added', async snap => {
    const pairKey = snap.key;
    if (!pairKey.includes(myUid)) return;
    const d = snap.val();
    if (!d?.offer || d.offerer === myUid) return; // We're the answerer
    const theirUid = pairKey.replace(myUid, '').replace('_', '');
    if (callState.pcs[theirUid]) return;
    ...
    const sigRef = rtdb.ref(`calls/${roomId}/signals/${pairKey}`);
    await sigRef.update({ answer: { sdp: answer.sdp, type: answer.type } });

    pc.onicecandidate = e => {
      if (e.candidate) sigRef.child('answerCandidates').push(e.candidate.toJSON());
    };

    sigRef.child('offerCandidates').on('child_added', async cSnap => {
      try { await pc.addIceCandidate(new RTCIceCandidate(cSnap.val())); } catch {}
    });
  });
}
...
// Watch for active call notifications
function watchActiveCall(callback) {
  if (!db) return;
  db.collection('activeCall').doc('current').onSnapshot(doc => {
