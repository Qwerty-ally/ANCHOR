...
function setupListeners() {
  ...
  db.collection('links').orderBy('createdAt', 'desc').onSnapshot(snap => {
    cache.links = snap.docs.map(d => ({ id: d.id, ...d.data() }));
    cache.loaded.links = true;
    _notify('links');
  });
  ...
  return new Promise((resolve, reject) => {
    ...
    xhr.onload = () => {
      try {
        ...
        else reject(new Error(`Cloudinary error ${xhr.status}: ${res.error?.message || xhr.responseText}`));
      } catch(e) { reject(new Error('Bad response: ' + xhr.responseText.substring(0,200))); }
    };
...
async function submitAssignment(assignmentId, userId, content, fileUrl, fileName) {
  const a = cache.assignments.find(x => x.id === assignmentId);
  if (!a) return false;
  const subs = [...(a.submissions || [])];
  const idx = subs.findIndex(s => s.userId === userId);
  const sub = { userId, content: content || '', submittedAt: new Date().toISOString(), status: 'submitted', fileUrl: fileUrl || null, fileName: fileName || null };
...
// ── LINKS ─────────────────────────────────────────────────────────
function getLinks() { return cache.links; }

async function addLink(addedBy, data) {
  const l = { addedBy, ...data, createdAt: firebase.firestore.FieldValue.serverTimestamp() };
  const ref = await db.collection('links').add(l);
  return { id: ref.id, ...l };
}

async function deleteLink(id) { await db.collection('links').doc(id).delete(); }
...
// ── CALL ENGINE (Jitsi Meet) ───────────────────────────────────────
const callState = { active: false, roomId: null };

async function startCall() {
  const me = getCurrentUser();
  const roomId = 'ANCHOR-' + (getSettings().groupName || 'Group').replace(/\s+/g,'-') + '-' + Date.now();
  ...
  await db.collection('activeCall').doc('current').set({
    ...
    starterName: me?.displayName || 'Unknown',
    startedAt: firebase.firestore.FieldValue.serverTimestamp()
  });
  if (window.onCallStarted) window.onCallStarted(roomId);
}
...
// toggleMute / toggleCamera no longer needed (Jitsi has its own controls)
function toggleMute() {}
